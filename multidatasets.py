...
target_domains = ['clipart', 'real', 'painting', 'sketch']
...
# Known Classes
known_class_names = [test_class_names[idx] for idx in known_index_dom]
...
class_names.sort()
train_prev_classnames = class_names[:48]
...
best_open_set_acc = 0
best_avg_acc = 0
accuracy_file_path = "./results/multidataset/clipart.txt"  
...
for epoch in range(num_epochs):
    ...
    TRAIN_MODEL_PATH = Path("./train_models/multidataset/clipart")
    TRAIN_MODEL_PATH.mkdir(parents=True, exist_ok=True)
    TRAIN_MODEL_NAME = f"clipart_{epoch+1}.pth"
    ...
    MODEL_PATH = "./train_models/multidataset/clipart"
    MODEL_NAME = f"clipart_{epoch+1}.pth"
